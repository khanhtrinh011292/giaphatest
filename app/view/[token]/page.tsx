...
export default async function PublicViewPage({
  ...
  const [{ data: family }, { data: persons }, { data: relationships }] = await Promise.all([
    ...
    supabase.from("persons").select("id, full_name, gender, birth_year, birth_month, birth_day, death_year, is_deceased, generation, birth_order, avatar_url, note, other_names").eq("family_id", link.family_id).order("generation").order("birth_order"),
